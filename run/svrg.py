...
class SVRG(Optimizer):
    ...
    def step(self, x, y, step, closure=None):
        ...
        if (self.snapshot_rand and np.random.rand() <= self.prob) or (
            not self.snapshot_rand and (step + 1) % self.steps_per_snapshot == 0
        ):  # coin flip
            self.take_snapshot()
    ...
    def take_snapshot(self):
        """Takes snapshot of current neural network"""
