...
fout.write(
    f"label,{','.join(['d'+str(i) for i in range(1,d+1)])}\n"
)  # put heading in the first line
for line in fin.readlines():
    line = line[:-1]
    label = line.split()[0]
    datapoint_dict = {
        token.split(":")[0]: token.split(":")[1] for token in line.split()[1:]
    }
    datapoint = [
        "" if str(i) not in datapoint_dict else datapoint_dict[str(i)]
